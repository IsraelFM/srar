  ...
  Event.findById(id)
    .then((data) => {
      if (!data)
        ...
        const { _id, track_situation, track_type, measures_taken } = data;
        res.send({ id: _id, track_situation, track_type, measures_taken });
      }
    })
    .catch(() => {
      res.status(500).send({
...
exports.findAll = (req, res) => {
  Event.find({})
    .then((data) => {
      res.status(200).send(
        data.map(({ _id, track_situation, track_type, measures_taken }) => {
          return { id: _id, track_situation, track_type, measures_taken };
        })
      );
    })
    .catch((err) => {
...
exports.delete = async (req, res) => {
  ...
  if (!event) {
    ...
    res.status(500).send({
      message:
        "This event could not be deleted because have an accident using it!",
    });
  }
};
